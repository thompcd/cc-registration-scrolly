<script>
	...
	import STHeader from "./layout/STHeader.svelte";
	import STFooter from "./layout/STFooter.svelte";
	...
	$: region = selected && metadata.creek.lookup ? metadata.creek.lookup[selected].parent : null; // Gets region code for 'selected'
	$: chartHighlighted = metadata.creek.array ? metadata.creek.array.map(d => d.code) : []; // Array of district codes in 'region'
	...
	// INITIALISATION CODE
	datasets.forEach(county => {
		getData(`./data/data_${county}.csv`)
		.then(arr => {
			let meta = arr.map(d => ({
				party: d.party,
				code: d.code,
			...
			metadata[county].array = meta;
			metadata[county].lookup = lookup;
			...
			data[county].indicators = indicators;
			...
			data[county].timeseries = timeseries;
		});
	});

	$: console.log(data.creek.indicators)
...
<STHeader filled={true} center={false} />

<Header bgcolor="#206095" bgfixed={true} theme="dark" center={false} short={true}>
	<h1>Creek County Bleeds Blue</h1>
	<p class="text-big" style="margin-top: 5px">
		Taking a quick look at the recent migration away from the Democratic party within the county
	</p>
...
<Section>
	<h2>Creek County voter registration</h2>
	<p>
		Since 2000, Creek County has moved away from Democrats as seen in the voter registration total per party
	</p>
</Section>
{#if data.creek.timeseries && data.creek.indicators}
<Media
	col="wide"
	grid="narrow" gap={20}
	caption='Source: <a href="https://oklahoma.gov/elections/voter-registration/voter-registration-statistics/voter-registration-statistics-archive.html">Oklahoma State Election Board</a>'
>
	{#each [...data.creek.indicators].sort((a, b) => b.pop - a.pop) as region}
	<div class="chart-sml">
		<LineChart
			data={data.creek.timeseries}
			xKey="year" yKey="value" zKey="code"
			color="lightgrey"
			lineWidth={1} xTicks={4} snapTicks={false}
			yFormatTick={d => (d / 1e3)} ySuffix="k"
			height={200} padding={{top: 0, bottom: 20, left: 30, right: 15}}
			selected={region.code}
			area={false} title={region.party}/>
	</div>
	{/each}
</Media>
{/if}
...
<Section>
	<h2>What caused the decrease?</h2>
	<p>
		The loss of the party has been stable over the last 20 years
	</p>
	<p>
...
<Media
	col="full"
	height={600}
	caption='Party Registration'
>
	<div class="chart-full">
		{#if data.creek.timeseries}
		<LineChart
			data={data.creek.timeseries}
			...
			yFormatTick={d => (d/1e3).toFixed(1)} ySuffix="k"
			select {selected} on:select={doSelect}
			hover {hovered} on:hover={doHover}
			highlighted={chartHighlighted}
			colorSelect="#206095" colorHighlight="#999"
			area={false} title="Creek County voter registration totals by party, 2002 to 2022"
